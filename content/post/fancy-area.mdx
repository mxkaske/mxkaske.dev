...
To do so, we select the word to be replaced programmatically by setting the `selectionStart` and `selectionEnd` textarea properties and replace it with the above command with the new value.

The reason for its usage: it easily supports **undo**.

> Feel free to [contact me](https://twitter.com/mxkaske) if you have simple _non-deprecated_ way to do the same.

The supported _handles_ (like @mxkaske, @shadcn,...) are statically written inside of the code (see [data.ts]()). There is a potential rework to use dynamic data inside of `Command` using the `Command.Loading` component while fetching data from your API.
...
The ‘Mention’ Component that is being used in the preview is a _copy cat_ of shadcn example. This is also why Avatar is being used.
